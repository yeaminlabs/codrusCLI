...
from codrus_cli.cli.export import _collect_recent_log_files, _session_time_range
...
class TestCollectRecentLogFiles:
    ...
    def test_collects_logs_near_export_time(self, tmp_path: Path):
        ...
        now = time.time()
        self._make_log(log_dir, "codrus.log", now - 3600)  # 1 hour ago
        self._make_log(log_dir, "codrus.old.log", now - 10 * 86400)  # 10 days ago

        session_dir = tmp_path / "session"
        session_dir.mkdir()

        with patch("codrus_cli.share.get_share_dir", return_value=tmp_path):
            files = _collect_recent_log_files(session_dir)

        names = {f.name for f in files}
        assert "codrus.log" in names
        assert "codrus.old.log" not in names
    ...
    def test_collects_logs_near_session_time(self, tmp_path: Path):
        ...
        self._make_log(log_dir, "codrus.session-era.log", session_time + 3600)
        # This log is way too old (30 days before session)
        self._make_log(log_dir, "codrus.ancient.log", session_time - 30 * 86400)
        # Current log (near export time)
        self._make_log(log_dir, "codrus.log", now - 60)
        ...
        names = {f.name for f in files}
        assert "codrus.session-era.log" in names  # group 1: near session
        assert "codrus.log" in names  # group 2: near export
        assert "codrus.ancient.log" not in names  # too old for both groups

    def test_no_wire_file_falls_back_to_export_time(self, tmp_path: Path):
        """Without wire.jsonl, only group 2 (export time) applies."""
        log_dir = self._setup_log_dir(tmp_path)
        now = time.time()
        self._make_log(log_dir, "codrus.log", now - 3600)
        self._make_log(log_dir, "codrus.old.log", now - 5 * 86400)
    ...
    def test_ignores_non_log_files(self, tmp_path: Path):
        log_dir = self._setup_log_dir(tmp_path)
        (log_dir / "codrus.log").write_text("log")
...
class TestToolExecutionLogging:
    ...
    async def test_toolset_tool_execution_error_logged(self):
        ...
        with patch("codrus_cli.soul.toolset.logger") as mock_logger:
...
class TestFileToolLogging:
    ...
    async def test_read_file_exception_logged(self, read_file_tool):
        from codrus_cli.tools.file.read import Params

        with (
            patch("codrus_cli.tools.file.read.logger") as mock_logger,
            patch("codrus_cli.tools.file.read.kaos_path_from_user_input") as mock_path,
        ):
    ...
    async def test_write_file_exception_logged(self, write_file_tool):
        from codrus_cli.tools.file.write import Params

        with (
            patch("codrus_cli.tools.file.write.logger") as mock_logger,
            patch("codrus_cli.tools.file.write.kaos_path_from_user_input") as mock_path,
        ):
            mock_path.side_effect = RuntimeError("Unexpected")
    ...
    async def test_glob_exception_logged(self, glob_tool):
        from codrus_cli.tools.file.glob import Params

        with (
            patch("codrus_cli.tools.file.glob.logger") as mock_logger,
            patch("codrus_cli.tools.file.glob.kaos_path_from_user_input") as mock_path,
        ):
            mock_path.side_effect = RuntimeError("Unexpected")
    ...
    async def test_replace_file_exception_logged(self, str_replace_file_tool):
        from codrus_cli.tools.file.replace import Edit, Params

        with (
            patch("codrus_cli.tools.file.replace.logger") as mock_logger,
            patch("codrus_cli.tools.file.replace.kaos_path_from_user_input") as mock_path,
        ):
            mock_path.side_effect = RuntimeError("Unexpected")
...
class TestSearchWebLogging:
    async def test_search_timeout_logged(self, search_web_tool):
        from codrus_cli.tools.web.search import Params
        from tests.conftest import tool_call_context

        with (
            tool_call_context("SearchWeb"),
            patch("codrus_cli.tools.web.search.logger") as mock_logger,
            patch("codrus_cli.tools.web.search.new_client_session") as mock_session,
        ):
...
class TestLLMLogging:
    def test_create_llm_missing_config_logged(self):
        ...
        with patch("codrus_cli.llm.logger") as mock_logger:
            from codrus_cli.llm import create_llm

            result = create_llm(
                LLMProvider(type="codrus", base_url="", api_key=SecretStr("")),
                LLMModel(provider="codrus", model="", max_context_size=100_000),
            )
...
class TestSubagentRunnerLogging:
    async def test_max_steps_reached_logged(self):
        from codrus_cli.soul import MaxStepsReached
        from codrus_cli.subagents.runner import run_soul_checked

        with (
            patch("codrus_cli.subagents.runner.run_soul") as mock_run_soul,
            patch("codrus_cli.subagents.runner.logger") as mock_logger,
        ):
