...
from codrus_cli.cli.plugin import _parse_git_url, _resolve_source
...
def test_parse_plugin_json_with_runtime(tmp_path: Path):
    plugin_dir = _write_plugin(
        tmp_path,
        {
            ...
            "version": "1.0.0",
            "runtime": {"host": "codrus-code", "host_version": "1.22.0"},
        },
    )
    spec = parse_plugin_json(plugin_dir / "plugin.json")
    assert spec.runtime is not None
    assert spec.runtime.host == "codrus-code"
...
def test_write_runtime(tmp_path: Path):
    ...
    runtime = PluginRuntime(host="codrus-code", host_version="1.22.0")
    write_runtime(plugin_dir, runtime)

    data = json.loads((plugin_dir / "plugin.json").read_text())
    assert data["runtime"]["host"] == "codrus-code"
