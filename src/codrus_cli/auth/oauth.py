...
KIMI_CODE_OAUTH_KEY = "oauth/codrus-code"
DEFAULT_OAUTH_HOST = "https://auth.codrus.com"
KEYRING_SERVICE = "codrus-code"
...
class _CrossProcessLock:
    """File-based lock that coordinates token refresh across codrus-cli processes.
...
def _apply_kimi_code_config(
    ...
    *,
    ...
    if platform is None:
        raise OAuthError("Codrus Code platform not found.")
...
async def login_kimi_code(
    ...
    platform = get_platform_by_id(KIMI_CODE_PLATFORM_ID)
    if platform is None:
        yield OAuthEvent("error", "Codrus Code platform is unavailable.")
        return
...
class OAuthManager:
    ...
    async def _refresh_tokens(
        ...
        *,
        ...
        async with self._refresh_lock:
            ...
            # codrus-cli instances (terminal, VS Code, web).
    ...
    def _apply_access_token(self, runtime: Runtime | None, access_token: str) -> None:
        ...
        assert isinstance(runtime.llm.chat_provider, Codrus), "Expected Codrus chat provider"
