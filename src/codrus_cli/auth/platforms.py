...
from codrus_cli.config import Config, LLMModel, load_config, save_config
...
class ModelInfo(BaseModel):
    ...
    def capabilities(self) -> set[ModelCapability]:
        ...
        if self.supports_video_in:
            caps.add("video_in")
        if self.id.lower().startswith("codrus-k2"):
...
def _kimi_code_base_url() -> str:
    if base_url := os.getenv("KIMI_CODE_BASE_URL"):
        return base_url
    return "https://api.codrus.com/coding/v1"
