...
from codrus_cli.utils.path import normalize_user_path
...
RG_VERSION = "15.0.0"
RG_BASE_URL = "http://cdn.codrus.com/binaries/codrus-cli/rg"
...
def _find_existing_rg(bin_name: str) -> Path | None:
    ...
    assert codrus_cli.__file__ is not None
    local_dep = Path(codrus_cli.__file__).parent / "deps" / "bin" / bin_name
...
async def _download_and_install_rg(bin_name: str) -> Path:
    ...
    async with new_client_session(timeout=download_timeout) as session:
        with tempfile.TemporaryDirectory(prefix="codrus-rg-") as tmpdir:
