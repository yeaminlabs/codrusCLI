...
from codrus_cli.tools.file.utils import MEDIA_SNIFF_BYTES, FileType, detect_file_type
...
from codrus_cli.wire.types import ImageURLPart, VideoURLPart
...
class ReadMediaFile(CallableTool2[Params]):
    ...
    async def _read_media(self, path: KaosPath, file_type: FileType) -> ToolReturnValue:
        ...
        match file_type.kind:
            ...
            case "video":
                data = await path.read_bytes()
                if (llm := self._runtime.llm) and isinstance(llm.chat_provider, Codrus):
