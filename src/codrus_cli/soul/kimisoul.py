...
from codrus_cli.background import build_active_task_snapshot
...
from codrus_cli.notifications import (
    NotificationView,
    build_notification_message,
    extract_notification_ids,
)
from codrus_cli.skill import Skill, read_skill_text
from codrus_cli.skill.flow import Flow, FlowEdge, FlowNode, parse_choice
from codrus_cli.soul import (
    LLMNotSet,
    LLMNotSupported,
    MaxStepsReached,
    Soul,
...
from codrus_cli.soul.dynamic_injection import (
    DynamicInjection,
...
from codrus_cli.soul.message import check_message, system, system_reminder, tool_result_to_message
from codrus_cli.soul.slash import registry as soul_slash_registry
from codrus_cli.soul.toolset import KimiToolset
from codrus_cli.tools.dmail import NAME as SendDMail_NAME
from codrus_cli.tools.utils import ToolRejectedError
from codrus_cli.utils.logging import logger
from codrus_cli.utils.slashcmd import SlashCommand, parse_slash_command_call
...
def _track_api_error(
    ...
    *,
    ...
    from codrus_cli.telemetry import get_current_trace_id, track
...
class KimiSoul:
    ...
    def __init__(
        ...
        *,
        ...
        if self._plan_session_id is not None and self._runtime.session.state.plan_slug is not None:
            from codrus_cli.tools.plan.heroes import seed_slug_cache
    ...
    def _bind_plan_mode_tools(self) -> None:
        ...
        from codrus_cli.tools.ask_user import AskUserQuestion
    ...
    def _ensure_plan_session_id(self) -> None:
        ...
        if self._plan_session_id is None:
            ...
            from codrus_cli.tools.plan.heroes import get_or_create_slug
    ...
    def get_plan_file_path(self) -> Path | None:
        ...
        if self._plan_session_id is None:
            return None
        from codrus_cli.tools.plan.heroes import get_plan_file_path
    ...
    def read_current_plan(self) -> str | None:
        ...
        if self._plan_session_id is None:
            return None
        from codrus_cli.tools.plan.heroes import read_plan_file
    ...
    async def run(
        ...
        *,
        ...
        try:
            ...
            from codrus_cli.soul.toolset import set_session_id
            ...
            wire_send(TurnBegin(user_input=user_input))
            turn_started = True
            from codrus_cli.telemetry import track as _track_telemetry
            ...
            if not command_call:
                ...
                if session.state.custom_title is None:
                    ...
                    if title:
                        from codrus_cli.session_state import (
                            load_session_state,
                            save_session_state,
                        )
            ...
            if turn_started:
                ...
                from codrus_cli.telemetry import track as _track_ended
    ...
    async def _agent_loop(self) -> TurnOutcome:
        ...
        if isinstance(self._agent.toolset, KimiToolset):
            ...
            try:
                ...
                if loading:
                    from codrus_cli.telemetry import track as _track_mcp
        ...
        while True:
            ...
            try:
                ...
                # --- StopFailure hook ---
                from codrus_cli.hooks import events as _hook_events
    ...
    def _compute_completion_overrides(
        ...
        *,
        ...
        Returns request-scoped Codrus generation overrides, or ``None`` if no overrides
        ...
        in ``Codrus.on_retryable_error`` therefore affects subsequent steps as intended.
        """
