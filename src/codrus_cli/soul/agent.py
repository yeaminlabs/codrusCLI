...
from codrus_cli.agentspec import load_agent_spec
...
from codrus_cli.exception import MCPConfigError, SystemPromptTemplateError
...
from codrus_cli.soul.approval import Approval, ApprovalState
from codrus_cli.soul.denwarenji import DenwaRenji
from codrus_cli.soul.toolset import KimiToolset
from codrus_cli.subagents.models import AgentTypeDefinition, ToolPolicy
...
from codrus_cli.utils.path import find_project_root, is_within_directory, list_directory
...
async def load_agents_md(work_dir: KaosPath) -> str | None:
    ...
    1. ``.codrus/AGENTS.md``  — project-local codrus config (highest priority)
    2. ``AGENTS.md``        — standard location
    3. ``agents.md``        — lowercase variant (mutually exclusive with 2)

    Within a single directory, ``.codrus/AGENTS.md`` and ``AGENTS.md``/``agents.md``
    are **both** loaded (with ``.codrus/`` first), but ``AGENTS.md`` and ``agents.md``
    ...
    for d in dirs:
        # .codrus/AGENTS.md is always checked independently (can coexist with root-level file)
        kimi_path = d / ".codrus" / "AGENTS.md"
...
async def load_agent(
    ...
    *,
    ...
    from codrus_cli.plugin.tool import load_plugin_tools
