...
class GitBashNotFoundError(RuntimeError):
    """Raised when codrus-cli runs on Windows but cannot locate git-bash.

    git-bash (from Git for Windows) is required because codrus-cli's Shell tool
    runs commands through bash, not PowerShell.
    """


_GIT_BASH_INSTALL_HINT = (
    "codrus-cli on Windows requires Git for Windows (https://git-scm.com/downloads/win) "
