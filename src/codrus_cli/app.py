...
from codrus_cli.auth.oauth import KIMI_CODE_OAUTH_KEY, OAuthManager, get_device_id
from codrus_cli.background.models import is_terminal_status
from codrus_cli.cli import InputFormat, OutputFormat
from codrus_cli.config import Config, LLMModel, LLMProvider, load_config
from codrus_cli.constant import VERSION
from codrus_cli.llm import augment_provider_with_env_vars, create_llm, model_display_name
...
from codrus_cli.utils.logging import logger, open_original_stderr, redirect_stderr_to_logger
from codrus_cli.utils.path import shorten_home
from codrus_cli.wire import Wire, WireUISide
from codrus_cli.wire.types import ApprovalRequest, ApprovalResponse, ContentPart, WireMessage
...
def _patch_session_id(record: dict[str, Any]) -> None:
    ...
    try:
        from codrus_cli.soul.toolset import get_session_id
...
async def _refresh_managed_models_silent(config: Config) -> None:
    from codrus_cli.auth.platforms import refresh_managed_models
...
class CodrusCLI:
    ...
    async def create(
        ...
        *,
        ...
        from codrus_cli.telemetry import attach_sink, set_context
        from codrus_cli.telemetry import disable as disable_telemetry
        ...
        from codrus_cli.telemetry import track, track_session_started_once
        from codrus_cli.telemetry.crash import install_asyncio_handler, set_phase
    ...
    async def shutdown_background_tasks(self) -> None:
        ...
        try:
            ...
            if fresh_targets:
                ...
                # ``codrus.log`` because ``redirect_stderr_to_logger`` has
    ...
    async def run(
        ...
        """
        Run the CodrusCLI powered by Codrus models instance without any UI and yield Wire messages directly.
    ...
    async def run_shell(
        ...
        """Run the CodrusCLI powered by Codrus models instance with shell UI."""
        from codrus_cli.ui.shell import Shell, WelcomeInfoItem
        ...
        if not self._runtime.llm:
            ...
            if model_name not in (
                "codrus-for-coding",
                "codrus-code",
            ) and not model_name.startswith("codrus-k2"):
                welcome_info.append(
                    WelcomeInfoItem(
                        name="Tip",
                        value="send /login to use Codrus for Coding",
                        level=WelcomeInfoItem.Level.WARN,
                    )
                )
    ...
    async def run_print(
        ...
        *,
        ...
        """Run the CodrusCLI powered by Codrus models instance with print UI."""
    ...
    async def run_acp(self) -> None:
        """Run the CodrusCLI powered by Codrus models instance as ACP server."""
        from codrus_cli.ui.acp import ACP
    ...
    async def run_wire_stdio(self) -> None:
        """Run the CodrusCLI powered by Codrus models instance as Wire server over stdio."""
