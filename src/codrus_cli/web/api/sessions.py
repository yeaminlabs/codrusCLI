...
from codrus_cli.wire.types import is_request
...
async def create_session(request: CreateSessionRequest | None = None) -> Session:
    ...
    return Session(
        session_id=UUID(codrus_cli_session.id),
        title=codrus_cli_session.title,
        ...
        session_dir=str(codrus_cli_session.dir),
    )
...
async def upload_session_file(
    ...
    session_dir = session.codrus_cli_session.dir
...
async def get_session_upload_file(
    ...
    uploads_dir = (session.codrus_cli_session.dir / "uploads").resolve()
...
async def get_session_file(
    ...
    work_dir = Path(str(session.codrus_cli_session.work_dir)).resolve()
...
def _update_last_session_id(session: JointSession) -> None:
    ...
    kimi_session = session.codrus_cli_session
...
async def delete_session(session_id: UUID, runner: CodrusCLIRunner = Depends(get_runner)) -> None:
    ...
    wd_meta = session.codrus_cli_session.work_dir_meta
...
async def fork_session_endpoint(
    ...
    """
    from codrus_cli.session_fork import fork_session as do_fork

    source_session = get_editable_session(session_id, runner)
    source_dir = source_session.codrus_cli_session.dir
    work_dir = source_session.codrus_cli_session.work_dir
...
async def session_stream(
    ...
    try:
        ...
        try:
            ...
            work_dir = Path(str(session.codrus_cli_session.work_dir))
