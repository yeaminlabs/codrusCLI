...
from codrus_cli.acp.kaos import ACPKaos
from codrus_cli.acp.mcp import acp_mcp_servers_to_mcp_config
...
from codrus_cli.llm import create_llm, derive_model_capabilities
...
class ACPServer:
        ...
        **kwargs: Any,
        ...
        self._auth_methods = [
            acp.schema.AuthMethod(
                id="login",
                name="Login with Codrus account",
                description=(
                    "Run `codrus login` command in the terminal, "
    ...
    def _check_auth(self) -> None:
        """Check if Codrus Code authentication is complete. Raise AUTH_REQUIRED if not."""
    ...
    async def set_session_model(self, model_id: str, session_id: str, **kwargs: Any) -> None:
        ...
        assert config.is_from_default_location, "`codrus acp` must use the default config location"
