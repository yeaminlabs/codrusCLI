...
from codrus_cli.approval_runtime import ApprovalRuntime
...
from codrus_cli.soul.toolset import KimiToolset, WireExternalTool
...
class WireServer:
    ...
    def _approval_runtime(self) -> ApprovalRuntime | None:
        if isinstance(self._soul, CodrusSoul):
    ...
    async def serve(self) -> None:
        ...
        if isinstance(self._soul, CodrusSoul) and self._soul.runtime.root_wire_hub is not None:
    ...
    async def _shutdown(self) -> None:
        ...
        if (
            isinstance(self._soul, CodrusSoul)
    ...
    async def _handle_initialize(
        ...
        toolset = None
        if isinstance(self._soul, CodrusSoul) and isinstance(self._soul.agent.toolset, KimiToolset):
        ...
        from codrus_cli.hooks.config import HOOK_EVENT_TYPES
        from codrus_cli.hooks.engine import WireHookHandle, WireHookSubscription
        from codrus_cli.soul import wire_send
        from codrus_cli.wire.protocol import WIRE_PROTOCOL_VERSION
        from codrus_cli.wire.types import HookResolved, HookTriggered
    ...
    def _sync_ask_user_tool_visibility(self, toolset: KimiToolset) -> None:
        ...
        from codrus_cli.tools.ask_user import NAME as ASK_USER_TOOL_NAME
    ...
    def _sync_plan_mode_tool_visibility(self, toolset: KimiToolset) -> None:
        ...
        from codrus_cli.tools.plan import NAME as EXIT_PLAN_MODE_TOOL_NAME
        from codrus_cli.tools.plan.enter import NAME as ENTER_PLAN_MODE_TOOL_NAME
    ...
    def _apply_wire_client_info(self, client: ClientInfo | None) -> None:
        ...
        if not isinstance(self._soul, CodrusSoul):
            return
        ...
        if isinstance(llm.chat_provider, Codrus):
            codrus_client = llm.chat_provider.client
            headers = dict(codrus_client._custom_headers)  # pyright: ignore[reportPrivateUsage]
            headers["User-Agent"] = f"{USER_AGENT}{ua_suffix}"
            codrus_client._custom_headers = headers  # pyright: ignore[reportPrivateUsage]
    ...
    async def _handle_prompt(
        ...
        runtime = self._soul.runtime if isinstance(self._soul, CodrusSoul) else None
        try:
            await run_soul(
                self._soul,
    ...
    async def _handle_steer(
        ...
        if not isinstance(self._soul, CodrusSoul) or not self._is_streaming:
            return JSONRPCErrorResponse(
                id=msg.id,
                error=JSONRPCErrorObject(
                    code=ErrorCodes.INVALID_STATE,
    ...
    async def _handle_replay(
        ...
        wire_file = self._soul.wire_file if isinstance(self._soul, CodrusSoul) else None
