...
cli = typer.Typer(
    ...
    epilog="""\b\
Documentation:        https://moonshotai.github.io/codrus-cli/\n
LLM friendly version: https://moonshotai.github.io/codrus-cli/llms.txt""",
    add_completion=False,
    context_settings={"help_option_names": ["-h", "--help"]},
    help="Codrus, your next CLI agent.",
)
...
def _version_callback(value: bool) -> None:
    if value:
        ...
        typer.echo(f"codrus, version {get_version()}")
        raise typer.Exit()
...
def codrus(
    ...
    config_file: Annotated[
        ...
        typer.Option(
            ...
            dir_okay=False,
            readable=True,
            help="Config TOML/JSON file to load. Default: ~/.codrus/config.toml.",
        ),
    ] = None,
    ...
    acp_mode: Annotated[
        ...
        typer.Option(
            "--acp",
            help="(Deprecated, use `codrus acp` instead) Run as ACP server.",
        ),
    ] = False,
    wire_mode: Annotated[
        bool,
    ...
    """Codrus, your next CLI agent."""
    import asyncio
    import contextlib
    import json

    from codrus_cli.utils.proctitle import init_process_name

    init_process_name("Codrus Code")
    ...
    async def _run(session_id: str | None, prefill_text: str | None = None) -> tuple[Session, int]:
        ...
        try:
            ...
            if local_add_dirs:
                from codrus_cli.utils.path import is_within_directory
    ...
    try:
        ...
        if debug:
            ...
            log_path = get_share_dir() / "logs" / "codrus.log"
    ...
    if switch_target in ("web", "vis"):
        from codrus_cli.utils.logging import restore_stderr
        ...
        from codrus_cli.utils.term import ensure_tty_sane
...
def login(
    ...
    """Login to your Codrus account."""
    import asyncio

    from rich.console import Console
    from rich.status import Status

    from codrus_cli.auth.oauth import login_kimi_code
...
def logout(
    ...
    """Logout from your Codrus account."""
    import asyncio

    from rich.console import Console

    from codrus_cli.auth.oauth import logout_kimi_code
    from codrus_cli.config import load_config
...
def term(
    ...
    """Run Toad TUI backed by CodrusCLI powered by Codrus models ACP server."""
...
@cli.command()
def acp():
    """Run CodrusCLI powered by Codrus models ACP server."""
    from codrus_cli.acp import acp_main
...
def background_task_worker(
    ...
    from codrus_cli.background import run_background_task_worker
    from codrus_cli.utils.proctitle import set_process_title

    set_process_title("codrus-code-bg-worker")

    from codrus_cli.app import enable_logging
...
def web_worker(session_id: str) -> None:
    ...
    from codrus_cli.app import enable_logging
    from codrus_cli.web.runner.worker import run_worker
...
if __name__ == "__main__":
    ...
    if "codrus_cli.cli" not in sys.modules:
        sys.modules["codrus_cli.cli"] = sys.modules[__name__]
