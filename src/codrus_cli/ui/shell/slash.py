...
from codrus_cli.auth.platforms import get_platform_name_for_provider, refresh_managed_models
...
from codrus_cli.utils.changelog import CHANGELOG
from codrus_cli.utils.slashcmd import SlashCommand, SlashCommandRegistry
...
def version(app: Shell, args: str):
    ...
    console.print(f"codrus, version {VERSION}")


@registry.command
async def model(app: Shell, args: str):
    """Switch LLM model or thinking mode"""
    from codrus_cli.llm import derive_model_capabilities
...
async def editor(app: Shell, args: str):
    ...
    from codrus_cli.utils.editor import get_editor_command
...
async def feedback(app: Shell, args: str):
    """Submit feedback to make CodrusCLI powered by Codrus models better"""
    import platform
    import webbrowser
    ...
    from codrus_cli.auth.platforms import get_platform_by_id, managed_provider_key
    from codrus_cli.constant import VERSION
    from codrus_cli.ui.shell.oauth import current_model_key
    from codrus_cli.utils.aiohttp import new_client_session

    ISSUE_URL = "https://github.com/MoonshotAI/codrus-cli/issues"
...
async def list_sessions(app: Shell, args: str):
    ...
    from codrus_cli.ui.shell.session_picker import SessionPickerApp
    ...
    if selected_work_dir != current_session.work_dir:
        cmd = f"codrus --work-dir {shlex.quote(str(selected_work_dir))} --session {selection}"
...
def theme(app: Shell, args: str):
    ...
    from codrus_cli.ui.theme import get_active_theme
...
def web(app: Shell, args: str):
    """Open Codrus Code Web UI in browser"""
    from codrus_cli.telemetry import track
...
def vis(app: Shell, args: str):
    """Open Codrus Agent Tracing Visualizer in browser"""
    from codrus_cli.telemetry import track
...
async def upgrade(app: Shell, args: str):
    """Install Codrus Code — the faster successor (migrates your config & sessions)"""
    from codrus_cli.telemetry import track
    ...
    console.print(
        "[bold]This will install the new Codrus Code by running:[/bold]\n"
        f"  [cyan]{cmd}[/cyan]\n"
    ...
    console.print(
        "\n[green]The new Codrus Code is installed ✓[/green]  "
        "Your config & sessions were migrated automatically.\n"
        "Open a [bold]new terminal[/bold] and run [bold]codrus[/bold] to start it.\n"
        f"[grey50](Verify with `{verify_command(sys.platform)}` — it should point "
        "inside ~/.codrus-code.)[/grey50]"
    )
...
async def undo(app: Shell, args: str):
    ...
    from codrus_cli.session_fork import enumerate_turns, fork_session
...
async def fork(app: Shell, args: str):
    ...
    from codrus_cli.session_fork import fork_session
