...
from codrus_cli.background import list_task_views
from codrus_cli.llm import model_display_name
from codrus_cli.notifications import NotificationManager, NotificationWatcher
...
from codrus_cli.ui.shell.mcp_status import render_mcp_prompt
...
from codrus_cli.ui.shell.replay import replay_recent_history
from codrus_cli.ui.shell.slash import SKILL_COMMAND_PREFIX, shell_mode_registry
from codrus_cli.ui.shell.slash import registry as shell_slash_registry
from codrus_cli.ui.shell.update import LATEST_VERSION_FILE, UpdateResult, do_update, semver_tuple
...
from codrus_cli.utils.logging import open_original_stderr
from codrus_cli.utils.signals import install_sigint_handler
from codrus_cli.utils.slashcmd import SlashCommand, SlashCommandCall, parse_slash_command_call
from codrus_cli.utils.subprocess_env import get_clean_env
from codrus_cli.utils.term import ensure_new_line, ensure_tty_sane
...
class Shell:
        ...
        panel = Panel(
            Group(
                ...
                Text(
                    ...
                    "Restart codrus in a valid directory to continue.",
                    style="dim",
                ),
            ),
    ...
    async def run(self, command: str | None = None) -> bool:
        ...
        _print_welcome_info(self.soul.name or "CodrusCLI powered by Codrus models", self._welcome_info)

        # Start telemetry periodic flush and disk retry
        from codrus_cli.telemetry import get_sink
        ...
        with CustomPromptSession(
            ...
            try:
                while True:
                    ...
                    from codrus_cli.ui.shell.visualize import InputAction, classify_input
    ...
    async def run_soul_command(self, user_input: str | list[ContentPart]) -> bool:
        ...
        try:
            ...
            if not isinstance(e, APIStatusError) or e.status_code not in (401, 402, 403):
                console.print(
                    "[dim]If this persists, run [bold]codrus export[/bold] and send the "
            ...
            console.print(
                ...
                "[dim]Run [bold]codrus export[/bold] and send the exported data to support "
    ...
    async def _run_btw_modal(
        ...
        """
        from codrus_cli.soul.btw import execute_side_question
        from codrus_cli.ui.shell.visualize import (
...
def _print_welcome_info(name: str, info_items: list[WelcomeInfoItem]) -> None:
    head = Text.from_markup("Welcome to CodrusCLI powered by Codrus models!")
    ...
    if LATEST_VERSION_FILE.exists():
        ...
        from codrus_cli.ui.shell.update import SKIPPED_VERSION_FILE
        from codrus_cli.utils.envvar import get_env_bool
