...
- ``builtin``: bundled with codrus-cli
- ``user``: from the user's home (``~/.codrus/skills``, ``~/.agents/skills``, ...)
- ``project``: from the current project's working directory
  (``<work_dir>/.codrus/skills``, ``<work_dir>/.agents/skills``, ...)
...
def get_builtin_skills_dir() -> Path:
    ...
    if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
        ...
        return Path(meipass) / "codrus_cli" / "skills"
...
def _get_user_brand_skills_dir_candidates() -> tuple[KaosPath, ...]:
    ...
    Brand group: ``~/.codrus/skills`` > ``~/.claude/skills`` > ``~/.codex/skills``
    """
    return (
        KaosPath.home() / ".codrus" / "skills",
...
async def find_user_skills_dirs(
    *,
    ...
    directories are included (priority order: codrus > claude > codex).
    """
    dirs: list[KaosPath] = []
    if merge_brands:
...
async def find_project_skills_dirs(
    ...
    *,
    ...
    of ``work_dir``), so launching codrus-cli from a subdirectory — for example
    ...
    directories are included (priority order: codrus > claude > codex).
    """
    from codrus_cli.utils.path import find_project_root
...
def _first_meaningful_line(content: str) -> str | None:
    ...
    :mod:`codrus_cli.utils.frontmatter`). A malformed frontmatter opener that
    ...
    """
    from codrus_cli.utils.frontmatter import strip_frontmatter
