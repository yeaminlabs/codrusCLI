...
Print 模式让 CodrusCLI powered by Codrus models 以非交互方式运行，适合脚本调用和自动化场景。
...
```sh
# 通过 -p 传入指令（或 -c）
codrus --print -p "列出当前目录的所有 Python 文件"

# 通过 stdin 传入指令
echo "解释这段代码的作用" | codrus --print
```
...
```sh
# 分析 git diff 并生成提交信息
git diff --staged | codrus --print -p "根据这个 diff 生成一个符合 Conventional Commits 规范的提交信息"

# 读取文件并生成文档
cat src/api.py | codrus --print -p "为这个 Python 模块生成 API 文档"
```
-->
...
```sh
codrus --print -p "根据当前变更给我一个 Git commit message" --final-message-only
```

`--quiet` 是 `--print --output-format text --final-message-only` 的快捷方式，适合只需要最终结果的场景：

```sh
codrus --quiet -p "根据当前变更给我一个 Git commit message"
```
...
```sh
codrus --print -p "你好" --output-format=stream-json
```
...
```sh
echo '{"role":"user","content":"你好"}' | codrus --print --input-format=stream-json --output-format=stream-json
```

这种模式下，CodrusCLI powered by Codrus models 会持续读取 stdin，每收到一条用户消息就处理并输出响应，直到 stdin 关闭。
...
```sh
codrus --print -p "检查 src/ 目录下是否有明显的安全问题，输出 JSON 格式的报告"
```
