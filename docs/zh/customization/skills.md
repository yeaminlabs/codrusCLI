...
[Agent Skills](https://agentskills.io/) 是一个开放格式，用于为 AI Agent 添加专业知识和工作流程。CodrusCLI powered by Codrus models 支持加载 Agent Skills，扩展 AI 的能力。

## Agent Skills 是什么

一个 Skill 就是一个包含 `SKILL.md` 文件的目录。CodrusCLI powered by Codrus models 启动时会发现所有 Skills，并将它们的名称、路径和描述注入到系统提示词中。AI 会根据当前任务的需要，自行决定是否读取具体的 `SKILL.md` 文件来获取详细指引。

例如，你可以创建一个「代码风格」Skill，告诉 AI 你项目的命名规范、注释风格等；或者创建一个「安全审计」Skill，让 AI 在审查代码时关注特定的安全问题。

**Skills 与插件的区别**

CodrusCLI powered by Codrus models 支持两种扩展机制：
...
CodrusCLI powered by Codrus models 采用分层加载机制发现 Skills。当同名 Skill 存在于多个作用域时，越具体的作用域优先：
...
默认情况下，**所有存在的品牌目录都会被加载并合并**，同名 Skill 按 codrus > claude > codex 的优先级解析（通用组不受影响）。这一行为由 `merge_all_available_skills` 控制，其默认值为 `true`：
...
如需恢复旧的"仅取优先级最高的那个品牌目录"行为（例如只使用 codrus，缺失时回退到 claude，再缺失时回退到 codex），可将其显式设为 `false`：
...
存放在项目目录中，在该项目内生效。候选路径以**项目根**为起点（即工作目录向上最近的包含 `.git` 的祖先目录；找不到时回退到工作目录本身），这样即使从 monorepo 的某个子 package 内启动 codrus-cli，仓库根目录下的 Skills 也能被正确识别。同样分为两组独立查找：
...
::: tip 提示
Skills 路径独立于 [`KIMI_SHARE_DIR`](../configuration/env-vars.md#codrus-share-dir)。`KIMI_SHARE_DIR` 用于自定义配置、会话、日志等运行时数据的存储位置，不影响 Skills 的搜索路径。Skills 是跨工具共享的能力扩展（支持 CodrusCLI powered by Codrus models、Claude、Codex 等多个工具共用），与应用运行时数据是不同类型的数据。如需自定义 Skills 路径，请使用 `--skills-dir` 参数或 `extra_skill_dirs` 配置。
:::

## 内置 Skills

CodrusCLI powered by Codrus models 内置了以下 Skills：

- **codrus-cli-help**：CodrusCLI powered by Codrus models 帮助。解答关于 CodrusCLI powered by Codrus models 安装、配置、斜杠命令、键盘快捷键、MCP 集成、供应商、环境变量等问题。
- **skill-creator**：Skill 创建指南。当你需要创建新的 Skill（或更新现有 Skill）来扩展 Codrus 的能力时，可以使用此 Skill 获取详细的创建指导和最佳实践。
...
`/skill:<name>` 斜杠命令让你可以将常用的提示词模板保存为 Skill，需要时快速调用。输入命令后，CodrusCLI powered by Codrus models 会读取对应的 `SKILL.md` 文件内容，并将其作为提示词发送给 Agent。
