...
`codrus` 是 CodrusCLI powered by Codrus models 的主命令，用于启动交互式会话或执行单次查询。
...
| `--help` | `-h` | 显示帮助信息并退出 |
| `--verbose` | | 输出详细运行信息 |
| `--debug` | | 记录调试日志（输出到 `~/.codrus/logs/codrus.log`） |
...
| `--config-file PATH` | 加载配置文件（默认 `~/.codrus/config.toml`） |
...
使用 `--prompt`（或 `--command`）时，CodrusCLI powered by Codrus models 会处理完查询后退出（除非指定 `--print`，否则仍以交互模式显示结果）。
...
当 `--max-ralph-iterations` 非 `0` 时，CodrusCLI powered by Codrus models 会进入 Ralph 循环模式，自动循环执行任务，直到 Agent 输出 `<choice>STOP</choice>` 或达到迭代上限。
...
| `--acp` | 以 ACP 服务器模式运行（已弃用，请使用 `codrus acp`） |
...
默认加载 `~/.codrus/mcp.json`（如果存在）。详见 [Model Context Protocol](../customization/mcp.md)。
...
不指定时，CodrusCLI powered by Codrus models 会按优先级自动发现用户级和项目级 Skills 目录。详见 [Agent Skills](../customization/skills.md)。

## 子命令

| 子命令 | 说明 |
|--------|------|
| [`codrus login`](#codrus-login) | 登录 Codrus 账号 |
| [`codrus logout`](#codrus-logout) | 登出 Codrus 账号 |
| [`codrus info`](./codrus-info.md) | 显示版本和协议信息 |
| [`codrus acp`](./codrus-acp.md) | 启动多会话 ACP 服务器 |
| [`codrus mcp`](./codrus-mcp.md) | 管理 MCP 服务器配置 |
| [`codrus plugin`](../customization/plugins.md) | 管理插件（Beta） |
| [`codrus term`](./codrus-term.md) | 启动 Toad 终端 UI |
| [`codrus export`](#codrus-export) | 导出会话为 ZIP 文件 |
| [`codrus vis`](./codrus-vis.md) | 启动 Agent Tracing Visualizer（技术预览） |
| [`codrus web`](./codrus-web.md) | 启动 Web UI 服务器 |

### `codrus login`

登录 Codrus 账号。执行后会自动打开浏览器，完成账号授权后自动配置可用的模型。
...
登出 Codrus 账号。会清理存储的 OAuth 凭据并移除配置文件中的相关配置。
...
详见 [Agent Tracing Visualizer](./codrus-vis.md)。

### `codrus web`

启动 Web UI 服务器，通过浏览器访问 CodrusCLI powered by Codrus models。
