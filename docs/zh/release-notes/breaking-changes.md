...
本页面记录 CodrusCLI powered by Codrus models 各版本中的破坏性变更及对应的迁移指引。

## 未发布

## 1.49.0

### Codrus 不再自动发送旧版 `reasoning_effort`

Codrus 的 Thinking 配置现在仅使用 `thinking.type`。`Codrus.with_thinking(...)` 不再自动把旧版 `reasoning_effort` 参数加入请求。

- **受影响**：通过 Kosong 的 Codrus 供应商连接仍要求 `reasoning_effort` 的旧版 Codrus 兼容端点的应用
- **迁移**：将端点升级为支持 `thinking.type`。如果旧版端点仍要求该参数，请通过 `with_generation_kwargs(reasoning_effort="...")` 显式传入

## 1.43.0

### MCP OAuth token 缓存迁移到 `~/.codrus/mcp-oauth/`

CodrusCLI powered by Codrus models 现在为 MCP 服务器使用 FastMCP 3 的持久化 OAuth 存储 API，并将 MCP OAuth token 存储在 `~/.codrus/mcp-oauth/`。旧 FastMCP 2.x 缓存位置中的 token 不会自动迁移。

- **受影响**：升级前已经授权过 OAuth MCP 服务器的用户
- **迁移**：如果 `codrus mcp list` 显示某个 OAuth 服务器需要授权，请运行一次 `codrus mcp auth <name>`，在 `~/.codrus/mcp-oauth/` 中创建新的 token。如果已存储的 token 失效或损坏，可先运行 `codrus mcp reset-auth <name>` 再重新授权
...
- **迁移**：
  ...
  2. 如果 `bash.exe` 在非标准位置，启动 codrus-cli 前把 `KIMI_CLI_GIT_BASH_PATH` 环境变量设为它的绝对路径
  ...
  5. 如果 codrus-cli 找不到 `bash.exe`，现在会在启动时打印安装提示并退出，而不是回退到 PowerShell
...
`merge_all_available_skills` 配置项的默认值从 `false` 改为 `true`。codrus-cli 现在默认会合并用户级和项目级所有已存在的品牌 Skills 目录（`.codrus/skills`、`.claude/skills`、`.codex/skills`），而不是仅使用找到的第一个。升级后，同时维护多个品牌目录（例如同时保留 `~/.codrus/skills` 和 `~/.claude/skills`）的用户会开箱即看到全部 Skills。
...
`codrus_cli.tools.multiagent` 下的 `CreateSubagent` 和 `Task` 工具已移除，由新的 `Agent` 工具替代。

- **受影响**：在 Agent YAML 中引用 `codrus_cli.tools.multiagent:Task` 或 `codrus_cli.tools.multiagent:CreateSubagent` 的自定义 Agent 配置
- **迁移**：在 Agent YAML 的 `allowed_tools` 中替换为 `codrus_cli.tools.agent:Agent`
...
从 `0.76` 升级后，Thinking 模式设置不再自动保留。此前保存在 `~/.codrus/codrus.json` 中的 `thinking` 状态不再使用，改为通过 `~/.codrus/config.toml` 中的 `default_thinking` 配置项管理，但不会自动从旧版 `metadata` 迁移。

- **受影响**：此前启用 Thinking 模式的用户
- **迁移**：升级后需重新设置 Thinking 模式：
  - 使用 `/model` 命令选择模型时设置 Thinking 模式（交互式）
  - 或手动在 `~/.codrus/config.toml` 中添加：
...
`--acp` 选项已弃用，请使用 `codrus acp` 子命令。

- **受影响**：使用 `codrus --acp` 的脚本和 IDE 配置
- **迁移**：`codrus --acp` → `codrus acp`
...
- **受影响**：使用 `~/.codrus/config.json` 的用户
- **迁移**：CodrusCLI powered by Codrus models 会自动读取旧的 JSON 配置，但建议手动迁移到 TOML 格式
- **新位置**：`~/.codrus/config.toml`
...
`Task` 工具从 `codrus_cli.tools.task` 移至 `codrus_cli.tools.multiagent` 模块。

- **受影响**：自定义工具中导入 `Task` 工具的代码
- **迁移**：将导入路径改为 `from codrus_cli.tools.multiagent import Task`
...
### 包名从 `ensoul` 改为 `codrus-cli`

- **受影响**：使用 `ensoul` 包名的代码或脚本
- **迁移**：
  - 安装：`pip install ensoul` → `pip install codrus-cli` 或 `uv tool install codrus-cli`
  - 命令：`ensoul` → `codrus`
