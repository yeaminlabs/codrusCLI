...
CodrusCLI powered by Codrus models 提供了丰富的交互功能，帮助你高效地与 AI 协作。

## Agent 与 Shell 模式

CodrusCLI powered by Codrus models 有两种输入模式：

- **Agent 模式**：默认模式，输入的内容会发送给 AI 处理
- **Shell 模式**：直接执行 Shell 命令，无需离开 CodrusCLI powered by Codrus models
...
- **启动参数**：使用 `codrus --plan` 直接以 Plan 模式启动新会话
...
::: tip 提示
Thinking 模式需要当前模型支持。部分模型（如 `codrus-k2-thinking-turbo`）始终使用 Thinking 模式，无法关闭。
:::
...
如果剪贴板中是图片，CodrusCLI powered by Codrus models 会将图片缓存到磁盘并在输入框中显示为 `[image:…]` 占位符。发送消息后，AI 可以看到并分析这张图片。如果剪贴板中是视频文件，其文件路径会以文本形式插入输入框。
...
斜杠命令是以 `/` 开头的特殊指令，用于执行 CodrusCLI powered by Codrus models 的内置功能，如 `/help`、`/login`、`/sessions` 等。输入 `/` 后会自动显示可用命令列表。完整的斜杠命令列表请参考 [斜杠命令参考](../reference/slash-commands.md)。

## @ 路径补全

在消息中输入 `@` 后，CodrusCLI powered by Codrus models 会自动补全工作目录中的文件和目录路径。这让你可以方便地引用项目中的文件：
...
当 AI 需要执行可能有影响的操作（如修改文件、运行命令）时，CodrusCLI powered by Codrus models 会请求你的确认。
...
如果你信任 AI 的操作，或者你正在安全的隔离环境中运行 CodrusCLI powered by Codrus models，可以启用「YOLO 模式」来自动批准所有工具调用：
